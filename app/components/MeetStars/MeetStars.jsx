          ...
          <div className="hidden md:flex space-x-4">
            {tabs.map((tab) => (
              <button
                ...
                className={`relative px-4 py-2 font-medium transition-colors ${activeTab === tab ? "text-purple-600" : "text-gray-600"}`}
              >
                {tab}
                {activeTab === tab && (
                  <motion.div
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {starData[activeTab]?.map((user, index) => {
            if (starData[activeTab].length <= 3 || index >= starData[activeTab].length - 3)
              return (
                <motion.div
                  key={user.id}
