...
export default function ArticlePage() {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        const response = await axios.get(`https://thevibes.tech/api/blogs?filters[id][$eq]=${id}&populate=*`);
        
        // ✅ Fix: Use setArticle (not setArticles)
  ...
  // ✅ Fix: Show loading if article is null
  if (!article) return <p className="text-center mt-10">Loading...</p>;

  // ✅ Fix: Extract image URL safely from Strapi response
  let imageUrl;
  if(article?.hero?.formats?.large?.url ) {
    imageUrl = "https://thevibes.tech"+article?.hero?.formats?.large?.url
  }
  else {
  ...
  return (
    ...
    <div className="relative z-10 mx-10 bg-white p-6 sm:w-1/2 m-auto rounded-t-[20px] -mt-24">
      ...
      <p className="w-[219.73px] mt-10 text-[#576e89] text-sm font-normal leading-[17.50px]">Published on {article.published}</p>
              ...
              {articles.map((article, index) => {
                ...
                >
                  <Image
                    width={392}
                    height={160}
                    src={`https://thevibes.tech${article.hero.formats.medium.url}`}
                    alt={article.title}
