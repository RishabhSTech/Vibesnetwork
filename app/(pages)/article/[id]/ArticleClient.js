...
export default function ArticleClient({ id, blogs }) {
  ...
  useEffect(() => {
    const found = blogs.find((b) => b.slug === id || b.id === id || b.id === Number(id));
    setArticle(found || null);
    setRelated(blogs.filter((b) => (found ? b.id !== found.id : true)).slice(0, 3));
    setLoading(false);
  }, [id, blogs]);
  ...
  return (
    <div className="flex flex-col justify-center items-center mx-auto my-10 bg-white overflow-hidden">
      <div className="relative w-full h-96">
        <Image
          src={article.image ? `https://admin.thevibes.academy/network-media/${article.image}` : "/images/default.png"}
          alt={article.title}
          layout="fill"
          objectFit="cover"
        />
      ...
      <div className="relative z-10 mx-10 bg-white p-6 sm:w-1/2 m-auto rounded-t-[20px] -mt-24">
        ...
        <p className="w-[219.73px] mt-10 text-[#576e89] text-sm font-normal leading-[17.50px]">
          Published on {article.created_at || "N/A"}
        </p>

        {/* Content */}
        <div className="prose prose-lg mt-10">
          <div className="mt-4 whitespace-pre-line" dangerouslySetInnerHTML={{ __html: article.content }} />
        </div>
        ...
        {related.map((rel) => (
          ...
          >
            <Image
              width={392}
              height={160}
              src={rel.image ? `https://admin.thevibes.academy/network-media/${rel.image}` : "/images/default.png"}
              alt={rel.title}
